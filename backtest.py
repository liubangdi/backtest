...
#     #     选出标的
#     today_data = today_data.sort_values('value',ascending = False)[:1]
...
#     if freq==20: # 每月更新直接用已有数据
#         days=list((ssdata.get_data(secid='430002.OC',start_date=start_date,end_date=end_date,field='open').sort_index()).index)
#         return (days)
    if freq == 1:
        days=list((ssdata.get_data(secid='000001.XSHE',start_date=start_date,end_date=end_date,field='open').sort_index()).index)
#         return (days)
#     if freq == 240 or freq == 80:
#         tdays=list((ssdata.get_data(secid='430002.OC',start_date=start_date,end_date=end_date,field='open').sort_index()).index)
#         days=[]
...
#         return (days)  
        ...
        tdays=list((ssdata.get_data(secid='000001.XSHE',start_date=start_date,end_date=end_date,field='open').sort_index()).index)
        days=[]
        for i in range(len(tdays)):
            if i % freq == 0:
                days.append(tdays[i])
...
#data_list=['000001.XSHE','002142.XSHE', '002807.XSHE','002839.XSHE','600000.XSHG', '600015.XSHG','600016.XSHG', '600036.XSHG', '600908.XSHG', '600919.XSHG', '600926.XSHG', '601009.XSHG', '601128.XSHG', '601166.XSHG', '601169.XSHG', '601229.XSHG', '601288.XSHG', '601328.XSHG', '601398.XSHG', '601818.XSHG', '601838.XSHG', '601939.XSHG','601988.XSHG','601997.XSHG', '601998.XSHG', 
        #   '603323.XSHG'][:10]
data_list=['000001.XSHE','002142.XSHE', '002807.XSHE','600000.XSHG']

# 设置调仓频率
freq=20 # 单位：日
order_days = get_order_days(freq)
# 交易日列表 
trade_days=list((ssdata.get_data(secid='000001.XSHE',start_date=start_date,end_date=end_date,field='open').sort_index()).index)

start_date=trade_days[0].strftime('%Y-%m-%d') # 实际开始日期 （包含本月）
end_date=trade_days[-1].strftime('%Y-%m-%d')  # 实际截止日期（不包含本月）
# 基准为新三板指数'899001.CSI'
#benchmark = pd.read_csv('899001.CSI.csv',index_col=0)[start_date:end_date]   # 基准csv文件
benchmark = ssdata.get_data(secid='000001.XSHE' ,start_date=start_date,end_date=end_date,field='open')#基准
# 设置起始资金、股票仓位、现金仓位
capital = []
position = 0.
cash = 1000000
